...
class VideoPlayer {
    ...
    inner class VideoThread: Thread() {
        ...
        override fun run() {
            ...
            if (degree == 0) {
                ...
                mCallback?.videoInfo(width, height, during, matrix)
            } else {
                Matrix.setRotateM(matrix, 0, 180f, 1f, 1f, 0f)
                mCallback?.videoInfo(height, width, during, matrix)
            }
    ...
    interface VideoPlayerCallback {
        ...
        fun videoInfo(width: Int, height: Int, during: Long, matrix: FloatArray)
